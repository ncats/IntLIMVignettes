...
summary <- IntLIM::PermutationSummary(permResults = perm.res, inputResults = myres.sig,
                              plot = TRUE)
...
myres.sig.pairs <- paste(myres.sig$Analyte1, myres.sig$Analyte2, sep = "__V__")
summary$SignificanceCountTable$allSignificantPairs <- 
  as.character(summary$SignificanceCountTable$allSignificantPairs)
original.pairs.count <- unlist(lapply(myres.sig.pairs, function(pair){
  freq <- 0
  if(pair %in% summary$SignificanceCountTable$allSignificantPairs){
    freq <- summary$SignificanceCountTable$Freq[
      which(summary$SignificanceCountTable$allSignificantPairs == pair)]
  }
  return(freq)
}))
original.pairs.df <- data.frame(Pair=myres.sig.pairs, 
                                Perm.Count=original.pairs.count)
ggplot(data=original.pairs.df, aes(x = reorder(Pair, Perm.Count), 
  ...
  ggplot2::ylim(0,nrow(perm.res[[1]]))
print(length(which(original.pairs.df$Perm.Count == 0)))
print(length(which(original.pairs.df$Perm.Count < 10)))
```
