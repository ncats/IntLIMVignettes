...
IntLIM::OutputResults(inputResults = myres10, filename = "bcresults10.csv")
```
...
# pdata <- read.csv(file.path(getwd(), "/BRCA_data/pData.csv"), row.names = 1)
...
genes <- unlist(lapply(as.character(summaryPairs$Pair), 
                       function(pair){
  return(strsplit(pair, "__V__")[[1]][1])
}))
metabolites <- unlist(lapply(as.character(summaryPairs$Pair), 
                       function(pair){
  return(strsplit(pair, "__V__")[[1]][2])
}))
print(summaryPairs[which(genes == "ASNS"),])
print(summaryPairs[which(genes == "SLC7A1"),])
print(summaryPairs[which(genes == "GPT2"),])
```
